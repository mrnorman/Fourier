...
!
  140 continue
      ...
      !$acc routine seq
      real(8), dimension(1) :: trigs
      integer :: mode, n, ifax(13), i
!
...
!     sort factors into ascending order
  ...
  110 continue
      ...
      !$acc routine seq
      real(8), dimension(*) :: trigs
      real(8) :: pi=real(2.0D0*asin(1.0D0),8)
      real(8) :: del, angle
      integer :: n, mode, l, nh, la
      imode=iabs(mode)
      nn=n
   ...
   50 continue
      ...
      !$acc routine seq
      real(8), dimension(*) :: a, work, trigs
      integer :: inc, jump, n, lot, ink, ia, ib, ja, jb
      real(8) :: c, s
!
...
!
   50 continue
      ...
      !$acc routine seq
      real(8), dimension(*) :: work, a, trigs
      integer :: inc, jump, n, lot, nh, nx, ink, ia, ib, ja, jb
      real(8) :: scale, c, s
!
...
!
   50 continue
     ...
     &   (a,b,c,d,trigs,inc1,inc2,inc3,inc4,lot,n,ifac,la)
      !$acc routine seq
      real(8), dimension(*) :: a, b, c, d, trigs
      real(8) :: sin36, cos36, sin72, cos72, sin60, c1, c2, s1, s2, c3, s3, c4, s4
!
...
!dir$ ivdep
      ...
      c(jb+j)=(a(ia+i)-0.5D0*(a(ib+i)+a(ic+i)))-(sin60*(b(ib+i)-b(ic+i)))
      c(jc+j)=(a(ia+i)-0.5D0*(a(ib+i)+a(ic+i)))+(sin60*(b(ib+i)-b(ic+i)))
      d(jb+j)=(b(ia+i)-0.5D0*(b(ib+i)+b(ic+i)))+(sin60*(a(ib+i)-a(ic+i)))
      d(jc+j)=(b(ia+i)-0.5D0*(b(ib+i)+b(ic+i)))-(sin60*(a(ib+i)-a(ic+i)))
      i=i+inc3
      j=j+inc4
   55 continue
      ibase=ibase+inc1
